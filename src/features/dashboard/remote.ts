 ...
 * current_streak/education_track/week_start/kp_at_week_start, and the
 * public.leaderboard view — see supabase/migrations/0001, 0002, 0008 in
 * the studium-website repo). Everything else on Home (Continue Studying,
 * Daily Case, Recommended, exam readiness/mastery %, days to exam, study
 * time) has no real per-user backend anywhere in the product yet — not
 * mobile, not studium-website either — so it stays mock; wiring those is
 * separate, later work once that backend actually exists.
 *
  ...
  useEffect(() => {
    ...
    (async () => {
      const [profileResult, leaderboardResult] = await Promise.all([
        supabase
          .from('profiles')
          .select('name, total_kp, current_streak, education_track, week_start, kp_at_week_start')
          .eq('id', userId)
          .maybeSingle(),
